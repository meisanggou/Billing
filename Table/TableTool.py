...
class TableToolManager:
    ...
    def __init__(self, mysql_host):
        ...
        self.conn = MySQLdb.connect(host=mysql_host, user="dms", passwd="gene_ac252", db="information_schema", charset='utf8')
        self.cursor = self.conn.cursor()
        self.db_name = "dms"

    def list_table(self):
        sql = "SELECT TABLE_NAME, CREATE_TIME,TABLE_COMMENT FROM TABLES WHERE TABLE_SCHEMA='%s' AND TABLE_TYPE='BASE TABLE';" % self.db_name
    ...
    def get_table_info(self, table_name):
        sql = "SELECT COLUMN_NAME, COLUMN_TYPE,COLUMN_KEY,COLUMN_DEFAULT,EXTRA,COLUMN_COMMENT,IS_NULLABLE " \
              "FROM columns WHERE TABLE_NAME='%s' AND TABLE_SCHEMA='%s';" % (table_name, self.db_name)
        self.cursor.execute(sql)
        column_info = []
        for item in self.cursor.fetchall():
